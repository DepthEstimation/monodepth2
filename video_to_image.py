...
parser.add_argument("--wd", type=str, required=True, help="path to working directory")
...
cmd = "ffmpeg -i {} -qscale:v 2 -vf scale=1280:-1 -start_number 0 {}/%10d.jpg"
...
for file in files:
    if os.path.isfile(file):
        ...
        if file_extension in [".mp4"]:
            new_dir = os.path.join("../frames", "".join(os.path.splitext(os.path.basename(file))[:-1]))
            if os.path.exists(new_dir) and os.path.isdir(new_dir):
                continue
            else:
                os.mkdir(new_dir)
                os.system(cmd.format(file, new_dir))
        else:
            print("not .mp4")
    else:
        print(file)

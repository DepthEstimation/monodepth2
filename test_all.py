...
for file_name in file_list:
    if os.path.isfile(os.path.join(folder_path, file_name)):
        ...
        command1 = f"ffmpeg -i {original_video_path}/{file_name} -qscale:v 2 {frames_path}/{base_name}/%010d.jpg"
        # -vf scale=1280:-1
        # 위에 옵션을 주면 해상도를 바꾼다
        # os.path.file_list
        ...
        if len(os.listdir(os.path.join(depth_path, base_name))) == 0:   # 폴더가 비어 있으면 command2 돌리고 뭔가 있으면 스킵
            ...
            store_video_info(file_path, total_time)
